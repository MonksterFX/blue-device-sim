...
func dataToArrayBuffer(_ data: Data, in context: JSContext) -> JSValue? {
...
func jsArrayBufferToData(_ value: JSValue) -> Data? {
    guard let length = value.forProperty("length")?.toInt32(), length > 0 else { return nil }

    var bytes = [UInt8]()
    for i in 0..<length {
        if let byte = value.atIndex(Int(i))?.toInt32() {
